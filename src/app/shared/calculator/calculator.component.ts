...
@Component({
    selector: 'dl-calculator',
    templateUrl: 'app/shared/calculator/calculator.html'
})
export class CalculatorComponent {

    lcdValue = '';
    lcdValueExpression = false;
    clearValue = 'AC';
    completePattern = new RegExp('[0-9-+*/.()]');
    numbersPattern = new RegExp('^[0-9]+$');
    lcdBackGroundColor = '#424242';
    parenthesisFlag = 0;
    expression = '=';
    ...
    roundValue(input: string) {
        let dotLocation: number = input.toString().indexOf('.');
        let flagDigit = false;
        ...
        if (dotLocation > -1 && input.length > 15) {
            ...
            for (let i = 0; i < afterDotString.length; i++) {
                if (parseInt(afterDotString[i], 10) > 0) { flagDigit = true; }
    ...
    evaluate() {
        let valueToEvaluate = this.lcdValue;
        let polishNotation = '';
        let solution = '';
